...
const planetoidBeltQuantity = (solarSystem, densityIndex) => {
  ...
  if (twoD6() >= 8) {
    let dm = 0;
    if (solarSystem.gasGiants > 0) dm += 1;
    if (solarSystem.starCount > 2) dm += 1;
    dm += densityIndexDM(densityIndex);

    const roll = twoD6() + dm;
    if (roll <= 6) planetoidBelts = 1;
    else if (roll <= 11) planetoidBelts = 2;
    else planetoidBelts = 3;
  }
...
const determineBeltComposition = (star, belt) => {
  ...
  while (total > 100) {
    if (belt.mType > 0) belt.mType = Math.max(0, belt.mType - (total - 100));
    else if (belt.sType > 0) belt.sType = Math.max(0, belt.sType - (total - 100));
...
const addSignificantBodies = (star, belt) => {
  ...
  if (belt.span < 0.1) bodies -= 4;
  if (belt.hzcoDeviation > 3) bodies += 2;
  ...
  bodies = twoD6() - 10;
  let dm = 0;
  if (belt.hzcoDeviation >= 2 && belt.hzcoDeviation <= 3) dm += 1;
  if (belt.hzcoDeviation > 3) dm += 3;
  if (belt.span > 1) dm += 1;
  bodies += (dm + 1) * (belt.bulk + 1);
  if (belt.span < 0.1) bodies /= 2;
