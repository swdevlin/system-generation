...
const isAgricultural = (planet) => {
  if (planet.atmosphere.code >= 4 && planet.atmosphere.code <= 9)
    if (planet.hydrographics.code >= 4 && planet.hydrographics.code <= 8)
      if (planet.population.code >= 5 && planet.population.code <= 7)
        return true;
...
const isFluidOceans = (planet) => {
  if ((planet.atmosphere.code >= 10 && planet.atmosphere.code <= 120) || planet.atmosphere.code >= 15)
    if (planet.hydrographics.code >= 1)
      return true;
...
const isGarden = (planet) => {
  ...
  if (planet.atmosphere.code === 5 || planet.atmosphere.code === 6 || planet.atmosphere.code === 8)
    if (planet.hydrographics.code >= 5 && planet.hydrographics.code <= 7)
      return true;
...
const isNonAgricultural = (planet) => {
  if (planet.atmosphere.code <= 3)
    if (planet.hydrographics.code <= 3)
      if (planet.population.code >= 6)
        return true;
...
const isRich = (planet) => {
  if (planet.atmosphere.code === 6 || planet.atmosphere.code === 8)
    if (planet.population.code >= 6 && planet.population.code <= 8)
      if (planet.governmentCode >= 4 && planet.governmentCode <= 9)
        return true;
...
const isWaterworld = (planet) => {
  if ([2,3,4,5,6,7,8,9,13,14].includes(planet.atmosphere.code))
    if (planet.hydrographics.code === 10)
      return true;
...
const assignTradeCodes = (planet) => {
  ...
  if (isAsteroid(planet))
    planet.tradeCodes.push('As');
  if (isBarren(planet))
    planet.tradeCodes.push('Ba');
  if (isDesert(planet))
    planet.tradeCodes.push('De');
  if (isFluidOceans(planet))
    planet.tradeCodes.push('Fl');
  if (isGarden(planet))
    planet.tradeCodes.push('Ga');
  if (isHighPopulation(planet))
    planet.tradeCodes.push('Hi');
  if (isHighTech(planet))
    planet.tradeCodes.push('Ht');
  if (isIceCapped(planet))
    planet.tradeCodes.push('Ic');
  if (isIndustrial(planet))
    planet.tradeCodes.push('In');
  if (isLowPopulation(planet))
    planet.tradeCodes.push('Lo');
  if (isLowTech(planet))
    planet.tradeCodes.push('Lt');
  if (isNonAgricultural(planet))
    planet.tradeCodes.push('Na');
  if (isNonIndustrial(planet))
    planet.tradeCodes.push('Ni');
  if (isPoor(planet))
    planet.tradeCodes.push('Po');
  if (isRich(planet))
    planet.tradeCodes.push('Ri');
  if (isVacuumWorld(planet))
    planet.tradeCodes.push('Va');
  if (isWaterworld(planet))
    planet.tradeCodes.push('Wa');
};
